...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = GetFrameHeight(); // Tamanho do quadrado
    const ImVec2 pos = window->DC.CursorPos;
    ...
    const ImVec2 max = pos + ImVec2(square_sz, square_sz);
    const float rounding = square_sz * 0.2f; // Arredondamento dos cantos

    // Renderiza o fundo da checkbox (quadrado de cantos arredondados)
    ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    window->DrawList->AddRectFilled(min, max, bg_col, rounding);
    ...
    if (mixed_value)
    {
        // Estado "indeterminado" (tristate)
        window->DrawList->AddRectFilled(min + ImVec2(3, 3), max - ImVec2(3, 3), check_col, rounding);
    }
    else if (*v)
    {
        // Desenha um "checkmark" arredondado **centralizado e maior**
        const float check_sz = square_sz * 0.4f; // Aumenta o tamanho do checkmark
        ImVec2 center = min + ImVec2(square_sz * 0.5f, square_sz * 0.5f); // Centro da checkbox

        // Posição do checkmark
        // O checkmark é desenhado a partir do centro da checkbox, com os pontos ajustados para maior visibilidade
        ImVec2 check_start = center + ImVec2(-check_sz * 0.6f, check_sz * 0.1f); // Início do checkmark
        ImVec2 check_mid = center + ImVec2(-check_sz * 0.2f, check_sz * 0.5f); // Meio do checkmark
        ImVec2 check_end = center + ImVec2(check_sz * 0.6f, -check_sz * 0.4f); // Fim do checkmark

        ImVec2 points[] = { check_start, check_mid, check_end };
        window->DrawList->AddPolyline(points, 3, check_col, 0, 3.0f); // Aumenta a espessura do checkmark
    }

    // Renderiza o texto da checkbox
